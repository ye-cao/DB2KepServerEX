...
- 按 **S7-300/400 经典非优化块**布局规则计算偏移：BOOL 按位紧凑排列，多字节类型落在偶数字节
...
- 生成的地址是按 S7-300/400 经典非优化块规则**推算**的，不保证与任何特定 PLC 程序的
  实际内存布局一致。**用于生产环境前请务必核对地址**，尤其是涉及写操作的标签。
